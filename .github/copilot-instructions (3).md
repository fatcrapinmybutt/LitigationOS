...
8. The central database is `C:\Users\andre\LitigationOS\litigation_context.db` (9.92 GB, 691+ tables) — pipeline writes, apps read.
9. The OMEGA subsystem added 29 tables (879K+ rows): omega_filesystem_map, omega_zip_intelligence, omega_legal_actions, omega_predictions, omega_deadlines, omega_court_assessment, omega_filing_readiness, omega_evidence_patterns, omega_authority_inventory, omega_violation_analysis, omega_damaging_quotes, omega_claim_evidence_map, omega_temporal_analysis, omega_value_classification, omega_zip_clusters, omega_dupe_analysis, omega_drive_audit, omega_revenue_projections, omega_scores, omega_accountability, omega_intelligence_gaps, omega_distill_log, omega_evolution_config, omega_evolution_log, omega_anomaly_log, omega_alerts, omega_emergency_templates, omega_daily_summaries, omega_asset_inventory.

---

## 🚨 Shell Session Management (HARD LIMITS — applies to ALL repos)

PowerShell sessions are finite OS resources. Violating these rules causes unrecoverable "Invalid shell ID" crashes.

1. **HARD LIMIT: Max 3 concurrent async shells** — count before creating. If 3+ exist, STOP one first.
2. **Chain related commands** with `&&` in one shell — never create separate shells for related operations.
3. **Stop completed shells IMMEDIATELY** — every dangling shell wastes a pool slot.
4. **Always use named shellIds** (`"build"`, `"test"`, `"git1"`) — never rely on auto-generated IDs.
5. **Pre-flight cleanup** before multi-step operations: `list_powershell` → stop stale → then proceed.
6. **Recovery**: If "Invalid shell ID" appears → stop ALL sessions → wait 5s → retry with new named shell.
7. **Never use `python -c "..."`** — write to a temp `.py` file instead. Backslashes and quotes break.
8. **Max 2 parallel sub-agents** when spawning via `task` tool — each consumes shell slots.
...
LitigationOS is a litigation intelligence system for Michigan family law (Pigors v. Watson). It has three integrated subsystems: a **16-phase Python data pipeline** with a **56-agent fleet**, a **local-first AI engine** (zero-network), and an **Electron+Next.js desktop app** with web and mobile frontends. All data stays local on Windows. The pipeline processes ~3.3GB of evidence across 6+ drives into court-ready filings.

**OMEGA Subsystem (OPERATION OMEGA — 14 phases, 92+ tasks, CONVERGENCE ACHIEVED):**
- 10-axis scoring engine: 19 legal actions scored across 6 forums (JTC Ω93, MSC Ω84, USDC Ω81)
- Monte Carlo predictive model: 72%-97% success probability range
- Filing execution: 7 filing templates (5 MSC/COA with real arguments, 1 JTC template, 1 USDC §1983) + 3 response warfare
- Quality reviewed: 16 corrections applied, citation format standardized to Michigan style, all facts cross-verified
- USDC §1983 Complaint: 7 counts, 5 defendants, $1.5M+ damages, judicial/qualified immunity defeated
- Evidence mining: 62,074 pattern hits, 1,045 legal citations, 117 intelligence gaps
- Self-evolution engine + observability dashboard + autonomous ops
- Desktop .exe BUILT: `00_SYSTEM/apps/desktop/dist/LitigationOS 1.0.0.exe` (105MB)
- Website: `00_SYSTEM/apps/website/` — 18 pages, dynamic dashboard, 10+ API endpoints, builds clean
- Android: `00_SYSTEM/apps/android/` — Expo app, deps installed, ready for EAS build
- CLI: `00_SYSTEM/cli/` — `litigos` command with 5 subcommands
- Neo4j: `00_SYSTEM/neo4j/` — schema + CSV exports + load script
- CI/CD: `.github/workflows/` — ci.yml, release.yml, db-backup.yml
- Backup: DB compressed at `SAFETY_SNAPSHOT_DB/litigation_context.db.7z` (1.42GB, SHA-256 verified)
- Court-ready export: `06_FILINGS/OMEGA_GENERATED/court_ready/` — .docx + .txt versions
...
|------|------|-------|-------|--------|
| A | 2024-001507-DC | 14th Circuit, Muskegon | Hon. Jenny L. McNeill | Custody — parent-child separation since Aug 8, 2025 (day count calculated dynamically) |
...
| E | Judicial Misconduct | JTC / MSC | Various | JTC complaint DRAFTED with 6 counts — READY TO FILE |
| F | COA 366810 | MI Court of Appeals | Panel TBD | Appeal brief DRAFTED with 4 issues — READY TO FILE |
| G | MSC Original Action | MI Supreme Court | TBD | 4 filings DRAFTED (Emergency App, Supt Control, Habeas, Mandamus) — READY TO FILE |

Additional: USDC Western District MI (§1983 DRAFTED — 4 counts). Calculate current parent-child separation day count from Aug 8, 2025.
...
### Court-Ready FINAL Filings (OMEGA-Generated at 06_FILINGS/OMEGA_GENERATED/)

| # | Document | Forum | Status |
|---|----------|-------|--------|
| 1 | Emergency Application for Leave to Appeal | MSC | ✅ Arguments drafted, citations verified |
| 2 | Complaint for Superintending Control | MSC | ✅ Arguments drafted, citations verified |
| 3 | Petition for Habeas Corpus | MSC | ✅ Arguments drafted, citations verified |
| 4 | Writ of Mandamus | MSC | ✅ Arguments drafted, citations verified |
| 5 | Appeal Brief (Case 366810) | COA | ✅ MCR 7.212 compliant, 4 issues |
| 6 | JTC Formal Complaint (6 counts) | JTC | ✅ Multi-count with evidence |
| 7 | Federal §1983 Complaint (4 counts) | USDC WD MI | ✅ Due process + liberty + equal protection |
...
### Key DB Tables (691+ tables, 9.92GB, 8.5M+ rows)
...
**OMEGA (29 tables)**: omega_legal_actions(19), omega_predictions(19), omega_court_assessment(19), omega_filing_readiness(14), omega_deadlines(36), omega_scores(14), omega_filesystem_map(873K), omega_zip_intelligence(2,671), omega_evidence_patterns(41), omega_authority_inventory(1,045), omega_violation_analysis(17), omega_damaging_quotes(24), omega_claim_evidence_map(653), omega_temporal_analysis(357), omega_value_classification(372), omega_zip_clusters(68), omega_dupe_analysis(152), omega_drive_audit(96), omega_intelligence_gaps(117), omega_revenue_projections(36), omega_accountability(131), omega_evolution_config, omega_evolution_log, omega_anomaly_log, omega_alerts(6), omega_distill_log, omega_emergency_templates, omega_daily_summaries, omega_asset_inventory
...
- **DB**: `C:\Users\andre\LitigationOS\litigation_context.db` (9.92 GB) — NOT `C:\Users\andre\litigation_context.db` (28KB stub)
...
- **OMEGA Pipeline**: `00_SYSTEM\omega\` (score engine, judicial analysis, predictions, filing executor, evidence miner, self-evolution)
- **OMEGA Apps**: Desktop (`00_SYSTEM\apps\desktop\`), Website (`00_SYSTEM\apps\website\`), Android (`00_SYSTEM\apps\android\`), CLI (`00_SYSTEM\cli\`)
- **OMEGA Distill**: `00_SYSTEM\distill\` (zip clustering, value classifier, dedup, drive audit)
- **OMEGA Filing Templates**: `06_FILINGS\OMEGA_GENERATED\` (7 filings + INDEX + quality review)
- **OMEGA Deploy**: `00_SYSTEM\deploy\` (Dockerfile, docker-compose, vercel.json, .env.example)
- **OMEGA Neo4j**: `00_SYSTEM\neo4j\` (export script, load script, schema, CSVs)
- **Skills Library**: `C:\Users\andre\skills\` (registry + templates + 3 documented skills)
- **Safety Backup**: `C:\Users\andre\SAFETY_SNAPSHOT_DB\litigation_context.db.7z` (1.42GB compressed)
...
- **MAX 2 background agents at a time** — prevents EAGAIN pipe buffer overflow
...
- If EAGAIN persists: Kill idle node/MCP processes, then retry. Monitor `system_error_patterns` table.
