...
class RuleWatcher:
    def __init__(self):
        ...
        cfg = os.path.join(self.root, "..", "..", "configs", "rules_sources.json")
        self.sources = json.load(open(cfg, "r")) if os.path.exists(cfg) else {}
        ...
        for name, url in self.sources.items():
            try:
                ...
                if len(snaps) >= 2:
                    ...
                    delta = list(diff)
                results[name] = {"url": url, "snapshot": snap_path, "delta_lines": len(delta) if delta else 0, "delta_preview": delta[:20] if delta else []}
