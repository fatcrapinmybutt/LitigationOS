...
# ------------------ Artifact download endpoints (safe) ------------------
...
def _safe_join(vault_root: Path, rel_path: str) -> Path:
    rel = Path(rel_path).as_posix().lstrip("/")
    p = (vault_root / rel).resolve()
    vr = vault_root.resolve()
    if not str(p).startswith(str(vr)):
        raise HTTPException(400, "Unsafe path.")
    parts = Path(rel).parts
    if not parts or parts[0] not in SAFE_SUBTREES:
        raise HTTPException(403, f"Path not in safe subtrees: {SAFE_SUBTREES}")
    return p

@app.get("/artifacts")
def list_artifacts(prefix: str = "90_REPORTS", limit: int = 200):
    if SETTINGS is None:
        raise HTTPException(400, "Not configured. POST /configure first.")
    vault = Path(SETTINGS.vault_root)
    basep = _safe_join(vault, prefix)
    if not basep.exists():
