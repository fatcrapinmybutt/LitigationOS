...
Usage:
  python worldfirst_builder.py --src . --out ../20260209_2314_AGENT_SPEC_PACK.zip

Notes:
  - Does NOT compute CRC/SHA for contents (per user preference).
  - Relies on zip structural verification and manifest cross-check (if present).
"""
...
def iter_files(src: Path):
    # Stable, deterministic ordering: directories then files lexicographically
    all_files = []
    for p in src.rglob("*"):
        if p.is_file():
            rel = p.relative_to(src).as_posix()
            # exclude existing output zips if inside src
