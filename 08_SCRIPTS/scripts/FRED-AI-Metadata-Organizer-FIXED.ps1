...
# Requirements: Word COM, PDF text extractor optional
...
$DryRun = $false  # Set to $true to simulate moves without applying
...
# === Helper: extract text from TXT or DOCX (PDF optional later) ===
function Get-FileText {
    ...
    if ($ext -eq ".txt") {
        ...
        return ""  # Placeholder for future PDF extractor
    } else {
        return ""
    }
}
...
foreach ($file in $files) {
    ...
    if (-not $matched) {
        ...
        $dest = Join-Path $NeedsReview $file.Name
        if (-not $DryRun) {
            Move-Item $file.FullName $dest -Force
        }

        Add-Content -Path $Log -Value "Unmatched content: $($file.FullName) → $dest"
        Write-Host "Unmatched: $($file.Name) → Needs_Review"
    }
}

Write-Host ""
Write-Host "Metadata-based file organization complete. See log for details."
