...
#!/usr/bin/env python3
"""
LitigationOS Compiler Runtime v1.1.1
- Adds explicit/auto Tesseract configuration so OCR works if tesseract.exe is in Downloads.

Inputs:
- nodes_merged.csv
- nodes_neo4j_admin.csv (optional)
- CONTRADICTION_MAP*.csv
- PDFs (*.pdf) optional: OCR/extract text for QuoteAtoms + citation extraction
- 555.txt optional: corpus text for citation extraction only
...
def build_action_report(path: Path, cm: pd.DataFrame) -> None:
    ...
    for _, r in cm.iterrows():
        ...
        lines.append("- Vehicles: will strengthen once AuthorityTriples (authority->vehicle mapping) are added.")
        lines.append("")
    path.write_text("\n".join(lines), encoding="utf-8")
...
def main() -> int:
    ...
    ap.add_argument("--tesseract-cmd", default="", help="Full path to tesseract.exe (optional). Overrides auto-detect.")
    args = ap.parse_args()
        ...
        for pdf in pdfs:
            try:
                payload = extract_pdf_text_and_optional_ocr(
                    ...
                    dpi=args.ocr_dpi,
                    lang=args.ocr_lang,
                    tesseract_cmd=(args.tesseract_cmd.strip() or None),
                )
    ...
    # Optional corpus text for citations
    txt = in_dir / "555.txt"
    if txt.exists():
        try:
