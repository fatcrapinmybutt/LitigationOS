...
Dependencies (install first):
  pip install chromadb sentence-transformers pdfminer.six pypdf pdfplumber pillow pytesseract rapidfuzz unstructured[all-docs] pydantic python-magic-bin tiktoken uvicorn fastapi watchdog
Optional:
  pip install easyocr
...
Usage
  ...
  python LAWFORGE_MASTER_UPGRADE_v1.py index --root "F:\\" --out "F:\\LegalResults\\index" --ocr yes

  # Keyword search
  python LAWFORGE_MASTER_UPGRADE_v1.py search --index "F:\\LegalResults\\index" --q "lease sewer EGLE"

  # Vector search
  python LAWFORGE_MASTER_UPGRADE_v1.py vsearch --index "F:\\LegalResults\\index" --q "unlawful rent increase and sewage violations"

  # Ask LLM using retrieved context
  python LAWFORGE_MASTER_UPGRADE_v1.py ask --index "F:\\LegalResults\\index" --q "Summarize the EGLE violations and rent overcharges" --provider openai

  # Run API server
  python LAWFORGE_MASTER_UPGRADE_v1.py serve --index "F:\\LegalResults\\index" --host 127.0.0.1 --port 8000
"""
import os, sys, re, io, time, json, math, hashlib, pathlib, subprocess, mimetypes, shutil, warnings
from dataclasses import dataclass
from typing import List, Dict, Optional, Iterable, Tuple

# ==== Globals ====
DEFAULT_PATTERN = r".*\.(pdf|txt|rtf|docx?|xlsx?|pptx?|csv|png|jpg|jpeg|tiff|bmp|gif|heic|mp3|wav|m4a|ogg)$"
IGNORE_DIRS = {'.git', '.cache', '__pycache__', 'node_modules', '.venv', 'venv', 'env', '.mypy_cache'}
CHUNK_TOKENS = 600
CHUNK_OVERLAP = 80
MAX_FILE_MB = 200
...
def is_binary_by_mime(path: str) -> bool:
    ...
    if not mime:
        return False
    return any(mime.startswith(p) for p in ["audio/", "video/", "application/x-executable"])
...
def chunk_text(s: str, tokens: int = CHUNK_TOKENS, overlap:int = CHUNK_OVERLAP) -> List[str]:
    words = s.split()
    approx_tok_per_word = 1  # already token-like
    stride = max(1, tokens - overlap)
    chunks = []
    i = 0
    while i < len(words):
        segment = " ".join(words[i:i+tokens])
        if segment.strip():
...
def extract_text_pdf(path: str) -> str:
    # Prefer pdfplumber then pdfminer
    if L["pdfplumber"]:
        try:
            out = []
...
def extract_text_generic(path: str) -> str:
    ...
    # Fall back to unstructured for office docs and others
        ...
        for dirpath, dirnames, filenames in os.walk(root):
            ...
            for fname in filenames:
                ...
                try:
                    ...
                    if not txt and ocr:
                        txt = extract_text_image(full)  # last-ditch for images
                    record = {
                        "sha256": h,
    ...
    with open(corpus_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            for i, chunk in enumerate(chunk_text(text)):
                ...
                texts.append(chunk)
                metas.append({"path": j["path"], "relpath": j["relpath"], "sha256": j["sha256"], "chunk": i})
                if len(ids) >= batch:
...
def answer_with_llm(context: str, question: str, provider: str = "openai", max_tokens:int=512) -> str:
    ...
    if provider == "openai":
        try:
            ...
            resp = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role":"system","content":"You are a Michigan litigation assistant. Use only the provided context."},
    ...
    if provider == "llama":
        try:
            ...
            prompt = f"You are a Michigan litigation assistant. Use only the provided context.\nContext:\n{context}\n\nQuestion: {question}\nAnswer:"
    ...
    if provider == "hf":
        try:
            ...
            prompt = f"You are a Michigan litigation assistant. Use only the provided context.\nContext:\n{context}\n\nQuestion: {question}\nAnswer:"
            gen = client.text_generation(prompt, model="meta-llama/Meta-Llama-3-8B-Instruct", max_new_tokens=max_tokens, temperature=0.2)
            return gen.strip()
        except Exception as e:
            return f"[hf-error] {e}"
...
def retrieve_then_answer(index_dir: str, q: str, top_k:int=6, provider:str="openai") -> Dict:
    ...
    for r in kw + vs:
        ...
        if not text and r.get("path"):
            # try to reload chunk from corpus if missing
            pass
        if text:
            ctx_parts.append(text.strip())
        if len(" ".join(ctx_parts)) > 16000:  # cap context size
            break
    context = "\n\n---\n\n".join(ctx_parts)
    answer = answer_with_llm(context, q, provider=provider)
    return {"answer": answer, "context_len": len(context), "chunks_used": len(ctx_parts)}
