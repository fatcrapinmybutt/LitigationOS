...
# ================================================
# 🧠 FRED-AI v2.1 – Full Infrastructure with Safe Cleanup
# Location: F:\FRED
...
# === 5. Enhanced Safe Cleanup of Empty Directories ===
Write-Host "`n🧹 Removing empty folders under F:\ safely..."

Get-ChildItem -Path "F:\" -Recurse -Directory -Force -ErrorAction SilentlyContinue |
Where-Object {
    ($_.FullName -notlike "$root*") -and 
    ((Get-ChildItem -Path $_.FullName -Recurse -Force -ErrorAction SilentlyContinue | Where-Object { -not $_.PSIsContainer }).Count -eq 0)
} | ForEach-Object {
    try {
        Remove-Item -Path $_.FullName -Recurse -Force -ErrorAction Stop
        Write-Host "🗑️ Removed empty: $($_.FullName)"
    } catch {
        Write-Host "⚠️ Skipped (error): $($_.FullName)"
    }
}

Write-Host "`n✅ FRED-AI v2.1 setup complete. Evidence Matrix + Cleanup successfully executed."
