            ...
            if file.lower().endswith(('.pdf', '.docx', '.txt', '.json')):
                ...
                if 'show cause' in file.lower() or 'order' in file.lower():
                    violations.append({
                        ...
                        "detail": f"Review for MCR 3.606(B), MCL 600.1711, and Benchbook §2.4(B)",
                        "file": file,
                        "path": path
                    })
                    ...
                    violations.append({
                        "type": "Motion Filing",
                        "detail": f"Validate MCR 2.119 compliance and form linkage",
                        "file": file,
                        "path": path
                    })
...
if __name__ == "__main__":
    ...
    print(f"🧠 Scan Complete: {len(events)} files scanned. {len(violations)} issues flagged.")