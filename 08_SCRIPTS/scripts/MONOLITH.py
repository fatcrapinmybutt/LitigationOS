...
Highlights
  ...
  • Threaded copy (configurable workers)
  • Resumable CSV+JSONL manifest with short progress stats
...
Quick Start (Windows, PowerShell)
  python bridge_drive_meek_intake_optimized.py ^
    ...
    --workers 6 --strategy hash --default-bucket M1 --min-bytes 4096

Minimal Run (defaults in script header)
  python bridge_drive_meek_intake_optimized.py

Recommended: Run on Python 3.11+ for best performance.
"""
...
def write_manifest_row(csv_writer, jsonl_file, row: Dict[str, str]) -> None:
        ...
        if target.exists():
            if strategy == "size":
                if samefile_quick(src_file, target):
                    skipped += 1
                    write_manifest_row(csv_writer, jsonl_file, {
    ...
    with futures.ThreadPoolExecutor(max_workers=max(1, args.workers)) as ex:
        jobs = [
            ex.submit(process_one, p, dest_m1, dest_m2, args.dry_run, args.strategy,
                      prior_hashes, csv_writer, jsonl_f, args.default_bucket)
            for p in candidates
        ]
        for i, job in enumerate(futures.as_completed(jobs), 1):
            c, s, e, err = job.result()
            copied += c; skipped += s; extracted += e; errors += err
            if i % 200 == 0:
                log("Progress: {}/{}  copied={} skipped={} extracted={} errors={}".format(
                    i, len(jobs), copied, skipped, extracted, errors
                ))
