...
class LitigationBridge:
    ...
    # ------------------------------------------------------------------
    # Evidence search (FTS5 with LIKE fallback)
    # ------------------------------------------------------------------
    ...
    def search_evidence(
        ...
        """Search evidence_quotes via FTS5, falling back to LIKE."""
        if not self._is_real:
            return []
        try:
            with self._connect() as conn:
                # Try FTS5 when a query is provided
                if query and query.strip():
                    try:
                        safe_query = query.replace('"', '""')
                        sql = (
                            "SELECT eq.id, eq.source_file, eq.quote_text, "
                            "       eq.page_number, eq.category, eq.lane, "
                            "       eq.relevance_score, eq.filing_refs, eq.tags "
                            "FROM evidence_quotes eq "
                            "JOIN evidence_fts ON evidence_fts.rowid = eq.id "
                ...
                if category:
                    sql += " AND category = ?"
                    params.append(category)
                sql += f" ORDER BY relevance_score DESC LIMIT {int(limit)}"
                return [dict(r) for r in conn.execute(sql, params).fetchall()]
        except Exception:
    ...
    def get_evidence_categories(self) -> List[str]:
        """Distinct evidence categories for filter dropdowns."""
        if not self._is_real:
            return []
        try:
            with self._connect() as conn:
            ...
            logger.debug("get_evidence_categories failed", exc_info=True)
            return []

    def get_evidence_lanes(self) -> List[str]:
        """Distinct lane identifiers across evidence_quotes."""
        if not self._is_real:
            return []
        try:
            with self._connect() as conn:
                rows = conn.execute(
                    ...
                    "WHERE lane IS NOT NULL AND lane != '' ORDER BY lane"
                ).fetchall()
                return [r["lane"] for r in rows]
        except Exception:
            logger.debug("get_evidence_lanes failed", exc_info=True)
            return []
    ...
    # ------------------------------------------------------------------
    # Filing packages (disk scan)
    # ------------------------------------------------------------------

    def get_filing_packages(self) -> List[Dict[str, Any]]:
        """Scan disk for actual filing package directories."""
        packages: List[Dict[str, Any]] = []
        pkg_dir = Path(r"C:\Users\andre\Desktop\LITIGATION_FILING_PACKAGE")
        if not pkg_dir.exists():
            return packages
        try:
            for d in sorted(pkg_dir.iterdir()):
                if not (d.is_dir() and d.name.startswith("PKG_F")):
                    continue
                files = [f for f in d.iterdir() if f.is_file()]
                main_filing = d / "01_MAIN_FILING.md"
                pkg: Dict[str, Any] = {
                    "id": d.name,
                    "path": str(d),
                    "name": d.name.replace("PKG_", "").replace("_", " "),
                    "has_main": main_filing.exists(),
                    "files": [f.name for f in files],
                    "file_count": len(files),
                    "size_kb": sum(f.stat().st_size for f in files) // 1024,
                }
                if main_filing.exists():
                    try:
                        with open(main_filing, "r", encoding="utf-8", errors="replace") as fh:
                            for line in fh:
                                if line.strip().startswith("# "):
                                    pkg["title"] = line.strip()[2:].strip()
                                    break
                    except Exception:
                        pass
                packages.append(pkg)
        except Exception:
            logger.debug("get_filing_packages failed", exc_info=True)
        return packages
