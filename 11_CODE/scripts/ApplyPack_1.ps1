...
function ConvertFrom-Jsonc([string]$Text) {
  ...
  return ($t | ConvertFrom-Json -Depth 200)
}

function Merge-JsonFile([string]$Path, [string]$JsonContent) {
  $patch = ConvertFrom-Jsonc $JsonContent
  $targetObj = $null
  if (Test-Path $Path) {
    $raw = Get-Content -Raw -Path $Path -Encoding UTF8
    try { $targetObj = ConvertFrom-Jsonc $raw } catch { $targetObj = [pscustomobject]@{} }
  } else {
    $targetObj = [pscustomobject]@{}
  }

  # Special-case tasks.json merge by task label (higher signal than array-union)
  if ((Split-Path $Path -Leaf) -eq "tasks.json" -and $targetObj.tasks -and $patch.tasks) {
    $byLabel = @{}
    foreach ($t in @($targetObj.tasks)) { if ($t.label) { $byLabel[$t.label] = $t } }
    foreach ($t in @($patch.tasks))     { if ($t.label) { $byLabel[$t.label] = $t } }
    $targetObj.tasks = @($byLabel.Values | Sort-Object label)

    foreach ($p in $patch.PSObject.Properties) {
      if ($p.Name -ne "tasks") { $targetObj | Add-Member -NotePropertyName $p.Name -NotePropertyValue $p.Value -Force }
    }

    $out = $targetObj | ConvertTo-Json -Depth 200
    Write-Text $Path $out
    return
  }

  $merged = DeepMerge $targetObj $patch
  $out = $merged | ConvertTo-Json -Depth 200
  Write-Text $Path $out
}
