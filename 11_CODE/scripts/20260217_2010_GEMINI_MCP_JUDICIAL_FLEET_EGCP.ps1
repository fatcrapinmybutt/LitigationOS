...
- Always produces deliverables. If required fields are not present in the corpus, the drafts
  use EVIDENCE-GRADE FIELD blocks (Grade A–E) + an Evidence Gap Log + validation plans (no halting).
...
# ---------------------------
# Deterministic meta extraction (no LLM) — returns UNRESOLVED when not found (EGCP)
# ---------------------------
function Build-CaseMetaJson([string]$ExtractedDir, [string]$OutJson) {
  ...
  foreach ($t in $txts) {
    ...
    # Provide top-level defaults for drafts (EGCP-friendly: value + grade + basis + validation_plan)
  $tc = ($hits.discovered_case_numbers | Select-Object -First 1)
  $tcVal = if ($tc) { $tc } else { "UNRESOLVED" }
  $tcGrade = if ($tc) { "B" } else { "E" }

  $county = ($hits.discovered_counties | Select-Object -First 1)
  $countyVal = if ($county) { $county } else { "UNRESOLVED" }
  $countyGrade = if ($county) { "B" } else { "E" }

  $judge = ($hits.discovered_judges | Select-Object -First 1)
  $judgeVal = if ($judge) { $judge } else { "UNRESOLVED" }
  $judgeGrade = if ($judge) { "B" } else { "E" }
  ...
  $meta = [ordered]@{
    ...
    # Structured fields for agents (preferred)
    draft_defaults = [ordered]@{
      petitioner_name  = [ordered]@{ value="Andrew J. Pigors";        grade="A"; basis="user-provided profile"; validation_plan="N/A" }
      petitioner_email = [ordered]@{ value="andrewjpigors@gmail.com"; grade="A"; basis="user-provided profile"; validation_plan="N/A" }
      petitioner_phone = [ordered]@{ value="231-260-1936";            grade="A"; basis="user-provided profile"; validation_plan="N/A" }

      petitioner_address = [ordered]@{
        value="UNRESOLVED"; grade="E"; basis="not located in deterministic scan"
        validation_plan="Search caption blocks/signature blocks/proofs of service for petitioner's mailing address. Query: 'Address' 'Mailing' 'City, MI' 'Petitioner' in extracted text."
      }

      respondent_name = [ordered]@{
        value="UNRESOLVED"; grade="E"; basis="not located in deterministic scan"
        validation_plan="Search for caption lines (look for 'v.'), party blocks, 'Respondent', or prior orders naming the opposing party."
      }

      respondent_address = [ordered]@{
        value="UNRESOLVED"; grade="E"; basis="not located in deterministic scan"
        validation_plan="Search proofs of service, party blocks, and caption addresses; extract from any filings that include service addresses."
      }

      trial_court_case_number = [ordered]@{
        value=$tcVal; grade=$tcGrade; basis="regex scan over extracted text"
        validation_plan="Confirm in the exact caption block or Register of Actions; extract full suffixes and court identifiers."
      }

      county = [ordered]@{
        value=$countyVal; grade=$countyGrade; basis="keyword scan over extracted text"
        validation_plan="Confirm in caption/order header; identify county and court (e.g., 14th Circuit, 60th District)."
      }

      judge = [ordered]@{
        value=$judgeVal; grade=$judgeGrade; basis="named-entity scan over extracted text"
        validation_plan="Confirm in order signature blocks and captions; capture full judicial title and division."
      }
    }

    # Flat fields for convenience in prompts (fallback)
    draft_defaults_flat = [ordered]@{
      petitioner_name="Andrew J. Pigors"
      petitioner_email="andrewjpigors@gmail.com"
      petitioner_phone="231-260-1936"
      petitioner_address="UNRESOLVED"
      respondent_name="UNRESOLVED"
      respondent_address="UNRESOLVED"
      trial_court_case_number=$tcVal
      county=$countyVal
      judge=$judgeVal
    }
  }
...
# ---------------------------
# Evidence-Gap normalization + log (post-process outputs so "UNKNOWN" never survives)
# ---------------------------
function Normalize-EvidenceGaps([string]$Path, [string]$DocName = "") {
  if (-not (Test-Path $Path)) { return @() }

  $raw = Get-Content -Raw -LiteralPath $Path

  # Normalize legacy phrasing if the model emits it
  $norm = $raw `
    -replace 'UNKNOWN \(not found in current corpus\)', 'UNRESOLVED (evidence not located in current corpus)' `
    -replace '\bVerification Addendum\b', 'Evidence Gap Log'

  if ($norm -ne $raw) {
    Set-Content -LiteralPath $Path -Value $norm -Encoding UTF8
  }
  ...
  for ($i=0; $i -lt $lines.Count; $i++) {
    $ln = $lines[$i]
    if ($ln -match 'UNRESOLVED \(evidence not located in current corpus\)' -or
        $ln -match 'EVIDENCE-GRADE FIELD:' -or
        $ln -match '\bEvidence Gap Log\b') {
      $hits += [ordered]@{
        doc   = $(if ($DocName) { $DocName } else { (Split-Path $Path -Leaf) })
        file  = $Path
        line  = ($i + 1)
        text  = $ln.Trim()
      }
...
function Write-EvidenceGapLog([object[]]$GapHits, [string]$OutPath, [string]$ScopeRoot) {
  $sb = New-Object System.Text.StringBuilder
  [void]$sb.AppendLine("# EVIDENCE_GAP_LOG")
  [void]$sb.AppendLine("")
  [void]$sb.AppendLine("This log lists evidence-grade gaps detected in generated deliverables.")
  [void]$sb.AppendLine("Key policy: EGCP (Evidence-Grade Completion Protocol) — drafts are always produced; gaps are disclosed and paired with a validation plan.")
  [void]$sb.AppendLine("")
  [void]$sb.AppendLine("## Quick search recipes")
  [void]$sb.AppendLine("Run these inside the extracted corpus to resolve caption/service gaps:")
  [void]$sb.AppendLine("- ripgrep: `rg -n \"\\bv\\.|Respondent|Plaintiff|Defendant|Petitioner\\b\" -S .`")
  [void]$sb.AppendLine("- addresses: `rg -n \"Address|Mailing|Street|Apt|MI\\s+\\d{5}\" -S .`")
  [void]$sb.AppendLine("- case numbers: `rg -n \"\\d{4}-\\d{6}-[A-Z]{2}\" -S .`")
  [void]$sb.AppendLine("- judges: `rg -n \"Judge|Hon\\.|J\\.?\\s*L\\.?\" -S .`")
  [void]$sb.AppendLine("")
  [void]$sb.AppendLine("## Gaps detected")
  if (-not $GapHits -or $GapHits.Count -eq 0) {
    [void]$sb.AppendLine("- None detected by the scanner (this does *not* guarantee completeness).")
  } else {
    $grouped = $GapHits | Group-Object doc
    foreach ($g in $grouped) {
      [void]$sb.AppendLine("")
      [void]$sb.AppendLine("### " + $g.Name)
      foreach ($h in ($g.Group | Sort-Object line)) {
        [void]$sb.AppendLine(("* Line {0}: {1}" -f $h.line, $h.text))
      }
      [void]$sb.AppendLine(("* File: `{0}`" -f ($g.Group[0].file)))
    }
  }
  [void]$sb.AppendLine("")
  [void]$sb.AppendLine("## Scope root")
  [void]$sb.AppendLine(("`{0}`" -f $ScopeRoot))
...
- Evidence-Grade Completion Protocol (EGCP) — replace "UNKNOWN" + blockers:
- Never stop because a required field is missing.
- Do NOT write "UNKNOWN" and do NOT leave blank placeholders.
- Always output a usable draft by completing the record *to the highest evidence-grade available*.

For any required field that is not directly supported by the corpus, insert an EVIDENCE-GRADE FIELD block:

EVIDENCE-GRADE FIELD: <FieldName>
- Value: <best-available value OR "UNRESOLVED">
- Grade:
  A = verbatim in corpus (quote + path)
  B = deterministically parsed from corpus (cite)
  C = inference from multiple corpus facts (explain + cite)
  D = candidate set (ranked) when multiple plausible values exist (cite)
  E = unresolved after search
- Basis: citations (file path + short snippet)
- Validation plan: exact search query + likely document type/location

Additionally:
- Maintain a per-document "Evidence Gap Log" listing all Grade D/E fields with the validation plan.
- If the missing field affects caption/service/jurisdiction, keep the draft valid by stating the limitation explicitly and
  presenting the next-evidence plan instead of halting.
...
- Never use bracket placeholders like [____]. Use EGCP: evidence-grade blocks with Value='UNRESOLVED' when necessary.
...
If a field is missing, apply EGCP: produce EVIDENCE-GRADE FIELD blocks with Grade D/E as needed.
Output: CAPTION_FIELD_RESOLUTION_TABLE + Evidence Gap Log.
'@},
...
Rules:
- Missing caption fields => insert EVIDENCE-GRADE FIELD blocks (Grade A–E) + Evidence Gap Log. Never halt.
...
- Relief
Include an Evidence Gap Log for any Grade D/E fields.
'@},
...
Use EVIDENCE-GRADE FIELD blocks as needed (Grade A–E) + Evidence Gap Log. Never halt.
'@},
...
- Proof of service template (addresses handled via EGCP: EVIDENCE-GRADE FIELD blocks; never halt)
...
- Missing critical attachments (Evidence Gap Log + validation plan)
'@},
...
If evidence is thin, produce an Evidence Gap Log entry (what to locate + exact search plan).
"@
...
"@
    ...
    $GapHits += Normalize-EvidenceGaps -Path $outPath -DocName $s.out

    if (-not ($contextFiles -contains $s.out)) { $contextFiles += $s.out }
  }

  Write-EvidenceGapLog -GapHits $GapHits -OutPath (Join-Path $OutDir "EVIDENCE_GAP_LOG.md") -ScopeRoot $CaseTarget
}
...
"@
  ...
  Write-EvidenceGapLog -GapHits $GapHits -OutPath (Join-Path $OutDir "EVIDENCE_GAP_LOG.md") -ScopeRoot $DocsRoot
}
