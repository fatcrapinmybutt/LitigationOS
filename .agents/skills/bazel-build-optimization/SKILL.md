...
### v1.1 (2026-03-11) — Session-Learned Enhancements

**NOT APPLICABLE to LitigationOS**: This project uses Python pip + pytest, not Bazel. This skill should only trigger when a WORKSPACE.bazel or BUILD.bazel file exists in the repo.

**Pre-Check Gate Added**: Before applying any Bazel optimization, verify:
1. `WORKSPACE.bazel` or `WORKSPACE` file exists in repo root
2. At least one `BUILD.bazel` or `BUILD` file exists
3. `.bazelrc` or `.bazelversion` is present
If none exist → SKIP this skill and inform the user it's not applicable.

**Python Monorepo Alternative**: For Python-heavy projects without Bazel, consider:
- `pip install -e ".[dev]"` for editable installs
- `pytest` with `--cov` for test + coverage
- `pyproject.toml` build system (PEP 517/518)
- `tox` or `nox` for multi-environment testing
- These are the build patterns used by LitigationOS (see `11_CODE/litigationos/`)

**Windows Compatibility Note**: Bazel on Windows requires MSYS2 or WSL for many rules.
LitigationOS runs on bare Windows with Python 3.12+ — no Unix toolchain assumed.
