...
ΩΩΩΩ “ANYTHING ELSE?” — NICHE / FORGOTTEN LAYERS (SPEC-GRADE APPEND PACK)
===============================================================================
Goal: Add *interoperability-grade* preservation, identity, transparency, legal-domain interchange, and graph constraint layers that are commonly “forgotten” but become decisive in (a) evidence credibility, (b) long-term reproducibility, and (c) cross-system portability.

--------------------------------------------------------------------------------
A) PRESERVATION / ARCHIVAL-QUALITY RECORDKEEPING (OAIS + PREMIS + BAGIT + METS/ALTO)
--------------------------------------------------------------------------------
1) **OAIS-style package lifecycle (SIP→AIP→DIP)**
   - What it adds: formalizes *Submission* (intake), *Archival* (sealed record), and *Dissemination* (court-ready export) packages.
   - Output artifacts:
     - `SIP_<run_id>.zip` (raw intake snapshot)
     - `AIP_<run_id>.zip` (sealed preservation package + fixity + metadata)
     - `DIP_<run_id>.zip` (court-facing bundle, minimized + indexed)
   - Stage insertion:
     - Graph→Steps: “SIP build” (intake boundary)
     - Validators: “AIP fixity + completeness”
     - Compilers/Draft factories: “DIP packager”
   - Standard alignment: OAIS Reference Model (ISO 14721). 

2) **PREMIS Preservation Metadata layer**
   - What it adds: durable metadata for *objects, events, agents, rights*—i.e., a normalized way to record “what happened to this evidence and who did it.”
   - Output artifacts:
     - `premis_events.jsonl` (append-only event log)
     - `premis_objects.jsonl` (file/object identities + fixity refs)
     - `premis_agents.jsonl` (actor identities)
   - Stage insertion:
     - Steps: emit PREMIS “events” for every transformation (OCR, split, redact, export)
     - Validators: verify required PREMIS events exist for each derived artifact
   - Standard alignment: PREMIS (Library of Congress). 

3) **BagIt packaging for evidence bundles**
   - What it adds: a simple, widely-used packaging convention for “payload + manifests + metadata,” ideal for shareable evidence sets.
   - Output artifacts:
     - `bagit.txt`, `bag-info.txt`
     - `manifest-sha256.txt`, `tagmanifest-sha256.txt`
   - Stage insertion:
     - Compilers: package any exportable “EvidenceSet” as BagIt
     - Validators: confirm manifests match payload
   - Standard alignment: BagIt (RFC 8493). 

4) **METS/ALTO for OCR layout provenance (page geometry + text)**
   - What it adds: preserves *where* text was found on a page (coordinates), enabling later highlight/review and audit of OCR.
   - Output artifacts:
     - `alto/page_<n>.xml` (OCR + layout)
     - `mets.xml` (structural map linking pages)
   - Stage insertion:
     - Atomization: produce ALTO per page alongside text shards
     - Validators: check that every OCR text shard has a page-level layout companion
   - Standard alignment: Library of Congress ALTO/METS ecosystem. 

5) **Format identification & profiling (PRONOM + DROID)**
   - What it adds: deterministic file-format IDs (not “.pdf guessed”), version detection, and preservation planning signals.
   - Output artifacts:
     - `format_profile.csv` (PUID, mime, size, modifiedTime)
     - `unknown_formats.csv` (triage list)
   - Stage insertion:
     - Intake steps: characterize everything before parsing
     - Validators: refuse “parse” stages for unknown/unsupported formats unless routed to fallback
   - Standard alignment: UK National Archives PRONOM + DROID. 

6) **Time-based web capture normalization (Memento)**
   - What it adds: a standardized way to record “this web claim as of time T” and retrieve versions (critical for web evidence that changes).
   - Output artifacts:
     - `web_captures/index.jsonl` (URL, capture_time, memento_uri)
   - Stage insertion:
     - Evidence atomization: any web source gets a capture record + replayable reference
   - Standard alignment: Memento protocol (RFC 7089). 

7) **Court-facing PDF accessibility + robustness (PDF/UA)**
   - What it adds: a high-quality PDF output gate that improves portability (screen readers, accessibility requirements), and reduces “bad PDF” rejection vectors.
   - Output artifacts:
     - `court_ready.pdf` + `pdfua_report.json`
   - Stage insertion:
     - Draft factories: final PDF emit step includes PDF/UA check (optional but high-value)
   - Standard alignment: ISO 14289 (PDF/UA). 

--------------------------------------------------------------------------------
B) LEGAL-DOMAIN INTERCHANGE (AKOMA NTOSO / LEGALRULEML)
--------------------------------------------------------------------------------
8) **LegalDocML / Akoma Ntoso export lane**
   - What it adds: a structured XML representation for legal documents (orders, pleadings, sections, citations) enabling interchange and machine analysis.
   - Output artifacts:
     - `legaldocml/<doc_id>.xml`
     - `legaldocml/citations.jsonl` (citation graph extracted from XML)
   - Stage insertion:
     - Compilers: export “final draft” to LegalDocML
     - Validators: schema validation + citation completeness checks
   - Standard alignment: OASIS LegalDocML / Akoma Ntoso. 

9) **LegalRuleML lane (computable rules / normative statements)**
   - What it adds: a portable representation for legal norms/rules so authority propositions can be serialized and checked across tools.
   - Output artifacts:
     - `legalruleml/<authority_id>.xml` (rules extracted/encoded)
   - Stage insertion:
     - AuthoritySnapshot ingestion: optional “computable rule” export for core propositions
   - Standard alignment: OASIS LegalRuleML Core v1.0. 

--------------------------------------------------------------------------------
C) TRUST, SIGNATURES, AND LONG-TERM VERIFIABILITY (PAdES/XAdES + ATTESTATION TOKENS)
--------------------------------------------------------------------------------
10) **PDF signature profile awareness (PAdES)**
   - What it adds: an explicit lane for cryptographic sealing of final PDFs (where you choose to sign), and validation artifacts proving signature status.
   - Output artifacts:
     - `signatures/pades_validation.json`
     - `signatures/pades_report.txt`
   - Stage insertion:
     - Final compiler: optional “sign” step
     - Validators: signature validation report generation
   - Standard alignment: ETSI PAdES profile family (PDF Advanced Electronic Signatures). 

11) **General AdES constraints (ETSI EN/TS 119 102-1 family)**
   - What it adds: “signature validation as code”: constraints-based verification for long-term verifiability.
   - Output artifacts:
     - `signatures/ades_constraints.yaml`
     - `signatures/ades_validation.json`
   - Stage insertion:
     - Validators: AdES validation step for any signed artifact
   - Standard alignment: ETSI TS 119 102-1. 

12) **Attestation token lane (RATS/EAT) for high-assurance runs**
   - What it adds: cryptographically-bound statements about *the environment and process* (e.g., “this run used validator version X”), which can be chained into your provenance.
   - Output artifacts:
     - `attest/eat_<run_id>.cbor` (or JSON if wrapped)
     - `attest/attestation_results.json`
   - Stage insertion:
     - Validators: “environment attestation” before allowing release builds
   - Standard alignment: IETF RATS EAT work. 

13) **SCITT transparency-log adapter for pipeline attestations**
   - What it adds: append-only transparency for attestations (a “public ledger” pattern; you can keep it local/private, but same interface).
   - Output artifacts:
     - `transparency/scitt_receipts.jsonl`
   - Stage insertion:
     - Provenance: after producing DSSE/SLSA-style attestations, register receipts
   - Standard alignment: IETF SCITT charter. 

--------------------------------------------------------------------------------
D) IDENTITY / AUTHENTICITY OF ACTORS (DID + VERIFIABLE CREDENTIALS)
--------------------------------------------------------------------------------
14) **Actor identity as first-class graph nodes (W3C DID)**
   - What it adds: a standardized identifier format for “who” (issuer/holder/verifier), enabling signed statements without central identity providers.
   - Output artifacts:
     - `identities/dids.jsonl` (DID docs, methods, keys metadata)
   - Stage insertion:
     - Provenance: PREMIS agents can be represented via DID
   - Standard alignment: W3C DID Core v1.0. 

15) **Credentialed assertions (W3C Verifiable Credentials v2.0)**
   - What it adds: a standardized way to package “claims with cryptographic verifiability” (e.g., who scanned a document, who notarized an affidavit, who witnessed a screenshot capture).
   - Output artifacts:
     - `credentials/<claim_id>.jsonld` (VC)
     - `credentials/presentations/<run_id>.jsonld` (VP)
   - Stage insertion:
     - Evidence capture: optional “credential issuance” step for high-stakes events
   - Standard alignment: VC Data Model v2.0. 

--------------------------------------------------------------------------------
E) EVENTING / OBSERVABILITY NORMALIZATION (CLOUDEVENTS)
--------------------------------------------------------------------------------
16) **CloudEvents envelope for *every* pipeline event**
   - What it adds: a standard event envelope so your pipeline can be observed, replayed, and integrated with other systems without bespoke log parsing.
   - Output artifacts:
     - `events/cloudevents.jsonl` (one CloudEvent per pipeline step)
   - Stage insertion:
     - Steps/Validators/Compilers: emit CloudEvents for start/stop/outputs/errors
   - Standard alignment: CloudEvents (CNCF). 

--------------------------------------------------------------------------------
F) GRAPH CONSTRAINTS + LINKED DATA INTEROP (SHACL + JSON-LD)
--------------------------------------------------------------------------------
17) **SHACL shape validation layer for knowledge graphs**
   - What it adds: a standard way to validate graph constraints (types, cardinality, required properties) as “shapes.”
   - Output artifacts:
     - `schemas/shacl/*.ttl`
     - `validation/shacl_report.ttl` + `shacl_report.json`
   - Stage insertion:
     - Validators: run SHACL in addition to JSON Schema for graph exports
   - Standard alignment: W3C SHACL. 

18) **JSON-LD export lane for linked-data portability**
   - What it adds: a graph serialization that can move outside Neo4j while preserving semantics and context.
   - Output artifacts:
     - `graph/export.jsonld`
     - `graph/context.jsonld`
   - Stage insertion:
     - Compilers: “graph export” emits JSON-LD alongside CSV/JSON
   - Standard alignment: W3C JSON-LD 1.1. 

--------------------------------------------------------------------------------
G) SECURITY POSTURE ARTIFACTS (CSAF + VEX)
--------------------------------------------------------------------------------
19) **Machine-readable advisories & VEX for your own system releases**
   - What it adds: a structured way to declare vulnerabilities and whether they impact your released artifacts, preventing “unknown risk” contamination.
   - Output artifacts:
     - `security/csaf_<release_id>.json`
     - `security/vex_<release_id>.json`
   - Stage insertion:
     - Release compiler: generate security advisory stubs + VEX posture for your build
   - Standard alignment: OASIS CSAF v2.0. 

--------------------------------------------------------------------------------
H) DIGITAL FORENSICS PLAYBOOK INJECTION (EVIDENCE-QUALITY WORKFLOWS)
--------------------------------------------------------------------------------
20) **Forensic capture discipline templates (NIST SP 800-86 / SP 800-101)**
   - What it adds: procedural “how to capture / preserve / examine / report” scaffolding that can be encoded into PCW/POs for evidence acquisition tasks.
   - Output artifacts:
     - `forensics/playbooks/nist_sp800-86_checklist.md`
     - `forensics/playbooks/nist_sp800-101_mobile_checklist.md`
   - Stage insertion:
     - Acquisition Plans: when evidence is missing, emit a forensic-grade capture checklist + required metadata fields
   - Source alignment: NIST SP 800-86; NIST SP 800-101 Rev.1. 

--------------------------------------------------------------------------------
ΩΩΩΩ “STAGE MAP” INSERTIONS — NEW STAGES TO ADD (MINIMAL-NAME, MAX-POWER)
--------------------------------------------------------------------------------
Add these *between* existing nodes of your stage chain (names are intentionally simple):

- **Graph → Steps → _Characterizers_ → Schemas → Validators → _Attestors_ → Compilers → _Packagers_ → Draft factories → _Interchange Exporters_ → _Transparency Receipters_**
  - **Characterizers**: DROID/PRONOM profiling; MIME/PUID truth before parse.
  - **Attestors**: EAT/SCITT receipts; binds “who/what/when/how” to the run outputs.
  - **Packagers**: BagIt + OAIS DIP/AIP; deterministic distribution/preservation.
  - **Interchange Exporters**: LegalDocML + JSON-LD; cross-tool and cross-court portability.
  - **Transparency Receipters**: optional SCITT-like receipts for attestations (even if local-only).

--------------------------------------------------------------------------------
ΩΩΩΩ “FORGOTTEN BUT DECISIVE” QA GATES (ADD THESE AS HARD/SEMI-HARD GATES)
--------------------------------------------------------------------------------
- **Format gate**: refuse processing unknown formats unless routed to fallback (PRONOM/DROID evidence).
- **Transform completeness gate**: every derived artifact must have:
  - (a) a PREMIS event,
  - (b) an input pointer list,
  - (c) a validator report.
- **Export equivalence gate**: DIP/LegalDocML/JSON-LD exports must reconcile counts:
  - nodes, rels, citations, exhibits, paragraphs, signatures.
- **Web capture gate**: any web evidence must have Memento-style time capture metadata.
- **Accessibility gate** (optional): final PDFs pass PDF/UA checks when feasible.

End of ΩΩΩΩ append pack.


===============================================================================
ΩΩΩΩΩ “ANYTHING ELSE?” • NICHE-BUT-HIGH-LEVERAGE ADD-ONS (V2026-01-19.6 APPEND)
===============================================================================

This section adds several “digital preservation + long-horizon integrity” primitives that are widely used in
institutional preservation and secure document distribution, and that complement the existing PROV/OpenLineage/
attestation/transparency layers.

-------------------------------------------------------------------------------
U) OCFL PLANE — Preservation-centric object layout (filesystem/cloud)
-------------------------------------------------------------------------------
- U0. Why: OCFL defines a stable, application-independent storage layout for versioned “objects-at-rest”.
- U1. Insert point:
  - Between S02 (Canonicalization) and S03 (Atomization), add **S02.9 — OCFL Object Builder**.
- U2. Outputs:
  - `ocfl/storage_root/` (the OCFL storage root)
  - `ocfl/objects/<object_id>/` (per-object versioned inventory)
  - `ocfl/index/object_index.csv` (object_id → path → latest_version → digest)
- U3. Invariants:
  - Every EvidenceSet/ContextPack/Packet may be stored as an OCFL object (one object per logical artifact family).
  - OCFL inventory is treated as “storage truth” for object versions, separate from the graph truth.
- U4. Graph hooks:
  - `(:OcflStorageRoot)` → `(:OcflObject {object_id})` → `(:OcflVersion {v})` → `(:Artifact {artifact_id})`.

-------------------------------------------------------------------------------
V) RO-CRATE PLANE — Interoperable “crate” packaging of a directory + metadata
-------------------------------------------------------------------------------
- V0. Why: RO-Crate is a lightweight packaging spec that couples a folder of files with machine-readable metadata
  (schema.org/JSON-LD), enabling external tools to understand what the folder contains.
- V1. Insert point:
  - After S16 (PacketIR/Binder Compiler), add **S16.9 — Crate Builder**.
- V2. Outputs:
  - `crate/ro-crate-metadata.json` (crate metadata)
  - `crate/ro-crate-preview.html` (optional preview)
  - `crate/manifest.csv` (crate file listing + digests)
- V3. Invariants:
  - The crate describes the Packet/Binder output: parties, case_id, run_id, artifact list, provenance pointers,
    licensing/rights metadata (even if “internal only”).
- V4. Graph hooks:
  - `(:Crate {crate_id})-[:DESCRIBES]->(:Packet)` and `(:Crate)-[:CONTAINS]->(:Artifact)`.

-------------------------------------------------------------------------------
W) LONG-TERM INTEGRITY PLANE — Evidence Record Syntax (ERS) + renewal
-------------------------------------------------------------------------------
- W0. Why: For multi-year retention, plain hashes or a single timestamp are weak against cryptographic expiry.
  ERS (RFC 4998) / XMLERS (RFC 6283) provide a structure for long-term non-repudiation of existence and integrity
  via archive timestamp chains, rehashing, and restamping patterns.
- W1. Insert points:
  - Add **S17.4 — EvidenceRecord Builder** (creates ERS/XMLERS records for key artifacts).
  - Add **S17.5 — EvidenceRecord Renewal** (periodic rehash/restamp policy).
- W2. Outputs:
  - `release/evidence_records/ers/*.ers` (ASN.1 / RFC 4998 form; if implemented)
  - `release/evidence_records/xmlers/*.xml` (RFC 6283 form; easier interoperability)
  - `release/evidence_records/policy.json` (renewal intervals + algorithms)
- W3. Invariants:
  - EvidenceRecord is treated as “long-horizon integrity spine” for: packet.zip, binder.pdf, manifest.csv,
    attestation envelopes, and any WARC captures.
- W4. Graph hooks:
  - `(:Artifact)-[:HAS_EVIDENCE_RECORD]->(:EvidenceRecord)` and `(:EvidenceRecord)-[:RESTAMPED_TO]->(:EvidenceRecord)`.

-------------------------------------------------------------------------------
X) SIGNATURE-CONTAINER PLANE — ASiC containers for signed bundles (optional)
-------------------------------------------------------------------------------
- X0. Why: ASiC (ETSI) defines ZIP-based “Associated Signature Containers” that bundle objects with detached signatures
  and/or time assertions—useful when you want “one file” portability while keeping signatures detached and auditable.
- X1. Insert point:
  - After Packet packaging, add **S16.8 — ASiC Packager** (optional).
- X2. Outputs:
  - `release/asic/<packet_id>.asice` or `.asics`
  - `release/asic/validation_report.json`
- X3. Validator:
  - Run AdES validation on the container and persist the report.

-------------------------------------------------------------------------------
Y) “VALIDATION ENGINE AS A TOOLCHAIN” — DSS validator hook (optional)
-------------------------------------------------------------------------------
- Y0. Why: The EU Digital Signature Service (DSS) provides a concrete validation toolchain for advanced signatures and
  containers (PAdES/XAdES/CAdES/ASiC). Even if you are not using EU trust services, the validator approach is valuable.
- Y1. Insert point:
  - As a general-purpose signature validation stage under S17 (ReleaseIR) and S16 (Packet).
- Y2. Outputs:
  - `validation/ades/*.json` (per-file validation traces)
  - `validation/ades/summary.json`

-------------------------------------------------------------------------------
Z) INTEGRATION RULE (ADD-ON)
-------------------------------------------------------------------------------
If you enable any of U–Y above:
- Every produced object MUST become an `Artifact` node.
- Every spec MUST have:
  - JSON Schema (when applicable) OR a formal spec reference + conformance profile.
  - A validator stage that emits a machine-readable report.
  - A provenance event linking inputs→outputs.
  - A stable ID strategy (artifact_id, object_id, crate_id).
- ERS/XMLERS records MUST reference:
  - the digests used,
  - the time assertions,
  - and the renewal policy snapshot.

-------------------------------------------------------------------------------
NON-QUOTED REFERENCES (PRIMARY SPECS)
-------------------------------------------------------------------------------
- OCFL specification: https://ocfl.io/1.0/spec/
- RO-Crate specification: https://www.researchobject.org/ro-crate/specification
- IETF RFC 4998 (ERS): https://datatracker.ietf.org/doc/html/rfc4998
- IETF RFC 6283 (XMLERS): https://www.rfc-editor.org/rfc/rfc6283.html
- ETSI ASiC (container standards): https://www.etsi.org/
- EU DSS validation docs: https://ec.europa.eu/digital-building-blocks/DSS/
---

# ΩΩΩΩΩ LAST CHECK — “MUST / SHOULD / WANT” APPEND SET (v2026-01-19.7)

> Goal: add the *missing* “standards-grade” planes that harden determinism, anchoring/quoting, preservation packaging, time-stamping, web capture, and event interoperability across the entire **Graph → Steps → Schemas → Validators → Compilers → Draft Factories** pipeline.

## A. MUST APPEND (Integrity, Determinism, Court-Facing Provenance)

### A1) Deterministic JSON canonicalization (for hash/signature stability)
- **Add Stage:** `Schemas → Validators → Compilers → Attestations`
- **What:** Adopt **JCS / JSON Canonicalization Scheme** for *every* signed/hashed JSON artifact (manifest.json, PO proofs, EvidenceAtom JSON, AuthoritySnapshot, RunLedger, etc.).
- **Why:** JSON text is not inherently canonical; without canonicalization, signatures/hashes can drift due to whitespace/member ordering differences.

### A2) Evidence “QuoteLock” anchoring as a first-class data model
- **Add Stage:** `Atomization → QuoteDB → EvidenceAtoms → Compilers`
- **What:** Add an **Annotation/Anchor** object type compatible with the W3C Web Annotation Model (selectors + target segments).
- **Minimum anchor set (text):**
  - TextQuoteSelector (exact quote + prefix/suffix)
  - TextPositionSelector (start/end offsets)
- **Minimum anchor set (audio/video/web):**
  - Media Fragment URI selectors (time ranges, spatial regions where applicable)
- **Why:** A QuoteLock system needs *portable*, *machine-checkable*, *lossless* references to exact snippets of source evidence—this is the core of “verbatim-correct” quoting at scale.

### A3) Evidence transfer + preservation packaging plane
- **Add Stage:** `Packager → Binder → Export → Long-term Store`
- **What:** Add two complementary packaging layers:
  - **BagIt** for transfer (checksums + payload manifest + standardized layout).
  - **OCFL** for durable object versioning (transparent version directories + predictable storage layout).
- **Why:** You need (1) a transport/package convention for moving bundles between machines, and (2) an “object versioning” convention for long-term preservation that supports append-only version growth.

### A4) “Time & Existence” cryptographic plane (timestamping + long-term proof)
- **Add Stage:** `Attestation → EvidenceChain → Nonrepudiation`
- **What (baseline):**
  - **RFC 3161** Time-Stamp Tokens for “time-of-existence” (TSA-backed time binding of a hash).
- **What (long-horizon):**
  - **RFC 4998** Evidence Record Syntax (ERS) for “long-term non-repudiation of existence of data”.
- **Why:** Courts and adversaries challenge integrity and timing; these standards give you durable, standard-structured answers.

### A5) Event interoperability envelope (every pipeline event is portable)
- **Add Stage:** `Signals → Scores → Gates → Actions → Observability`
- **What:** Standardize your event payloads on **CloudEvents** (Run events, Ingest events, Validation events, Compile events, Gate transitions, Export events).
- **Why:** You avoid bespoke event formats and gain tooling/interop across schedulers, log collectors, and downstream visualizers.

### A6) Web evidence capture & time travel plane
- **Add Stage:** `Ingest (Web) → Capture → EvidenceAtoms → Binder`
- **What:**
  - Archive web pages (HTML + resources) as **WARC** containers.
  - Add “past-state retrieval” hooks using **Memento** (TimeMaps / datetime negotiation) as an optional enrichment layer when the evidence is web-hosted.
- **Why:** Web evidence changes. WARC gives you a replayable archive; Memento gives you protocol-level access patterns to prior states when available.

---

## B. SHOULD APPEND (Interoperability, Graph Validity, Interchange)

### B1) CSV schema plane for node/edge tables
- **Add Stage:** `Schemas → Validators → Importers`
- **What:** Apply **Frictionless Table Schema** to:
  - nodes.csv, edges.csv, authority_triples.csv, exhibit_matrix.csv, timeline.csv, etc.
- **Why:** Table Schema gives you a portable, declarative schema for tabular datasets (types, required fields, constraints) so your CSVs are not “informal contracts”.

### B2) Rich, portable metadata crates for every bundle
- **Add Stage:** `Packager → Binder → Export`
- **What:** Add **RO-Crate** metadata per bundle (crate root + JSON-LD metadata file).
- **Why:** Your bundle becomes self-describing, indexable, and tool-friendly for long-term management and reuse.

### B3) “Linked data” optional interchange plane (only if you need semantic web interop)
- **Add Stage:** `Graph → Exporters → External Interchange`
- **What:**
  - Use **JSON-LD 1.1** as an optional export encoding for selected artifacts (authority triples, provenance edges, entity dictionaries).
  - Use **SHACL** as an optional validation/constraint layer for RDF/JSON-LD exports.
- **Why:** This lets you interoperate with semantic/knowledge graph tooling outside Neo4j if/when it is strategically useful.

### B4) Software supply-chain disclosures for your distributable tools (SBOM + VEX)
- **Add Stage:** `Compiler → Packager → Release`
- **What:**
  - SBOM: **SPDX** and/or **CycloneDX**
  - VEX: **OpenVEX**
- **Why:** If you distribute executables/scripts, you can (a) enumerate components, and (b) record exploitability statements. This is optional but becomes important if you want hardened/defensible distribution pipelines.

### B5) Formal provenance + lineage + telemetry “triangulation”
- **Add Stage:** `RunLedger → Provenance → Observability → Audit`
- **What:**
  - **PROV-DM** as the conceptual provenance schema
  - **OpenLineage** as the operational lineage event model
  - **OpenTelemetry** for cross-language telemetry correlation (traces/logs/metrics)
- **Why:** Together, these allow (1) *semantic* provenance, (2) *workflow* lineage, and (3) *systems* telemetry—so failures become traceable and reproducible rather than “mystery bugs”.

---

## C. WANT APPEND (Niche / Edge-Cases / Extra Power)

### C1) “Web evidence continuity” enrichment pack
- Add: WARC + timestamp token (RFC 3161) + embedded Web Annotation anchors for quotes extracted from web captures.
- Adds a clean chain: `Capture → Store → Quote → Verify → Present`.

### C2) “Event-sourced everything” hardening pack
- Use CloudEvents envelopes + append-only event log + deterministic replay to regenerate:
  - derived artifacts (indexes, matrices, binders),
  - graph deltas (nodes/edges),
  - and audit outputs (VR reports).

### C3) “Preservation-grade bundle profile”
- Use: BagIt (transfer), OCFL (object versioning), RO-Crate (metadata), ERS (long-term proof), plus explicit “read-only” exports for court production.

### C4) Optional “future interoperability” plane
- JSON-LD export + SHACL constraints for select artifacts only (AuthorityTriples, PROV graphs), kept separate from core Neo4j ingest.

---

## D. WHERE THESE LAND IN YOUR EXISTING STAGE MAP (Fast wiring)

- **Schemas/Contracts Layer:** JCS; Table Schema; JSON-LD; SHACL
- **Atomization/Quote Layer:** Web Annotation anchors; Media Fragments selectors
- **Packaging/Preservation Layer:** BagIt; OCFL; RO-Crate
- **Time/Nonrepudiation Layer:** RFC 3161; RFC 4998
- **Event/Observability Layer:** CloudEvents; PROV-DM; OpenLineage; OpenTelemetry
- **Supply Chain Layer (optional):** SPDX/CycloneDX; OpenVEX

---

## E. NON-NEGOTIABLE “SPEC-GRADE” INVARIANTS (If you append these planes)

- **Determinism:** every hash/signature uses canonicalization (JCS) + stable ordering + stable line endings.
- **Anchorability:** every quote has selectors; every selector round-trips to the source.
- **Reproducibility:** every compiled artifact can be regenerated from the event log + source store.
- **Transparency:** preservation bundles are inspectable without special tooling (BagIt/OCFL + plain manifests).
- **Auditability:** provenance is queryable (PROV-DM compatible) and linked to runs (OpenLineage/OTel correlation).
