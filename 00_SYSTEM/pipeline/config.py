...
# ═════════════════════════════════════════════════════════════════════
#  AGENT FLEET CONFIGURATION — Centralized tunables
# ═════════════════════════════════════════════════════════════════════
# Override via env vars: LITIGOS_AGENT_ITEM_TIMEOUT=60 etc.
AGENT_CONFIG = {
    # Per-item processing
    "item_timeout": int(os.environ.get("LITIGOS_AGENT_ITEM_TIMEOUT", "30")),
    "deadman_timeout": int(os.environ.get("LITIGOS_AGENT_DEADMAN_TIMEOUT", "120")),
    "checkpoint_interval": int(os.environ.get("LITIGOS_AGENT_CHECKPOINT_INTERVAL", "500")),
    "max_retries": int(os.environ.get("LITIGOS_AGENT_MAX_RETRIES", "3")),

    # Concurrency
    "max_db_connections": int(os.environ.get("LITIGOS_MAX_DB_CONNECTIONS", "3")),
    "db_lock_timeout": int(os.environ.get("LITIGOS_DB_LOCK_TIMEOUT", "60")),
    "db_busy_timeout_ms": int(os.environ.get("LITIGOS_DB_BUSY_TIMEOUT_MS", "120000")),

    # Circuit breaker
    "circuit_breaker_threshold": int(os.environ.get("LITIGOS_CB_THRESHOLD", "5")),
    "circuit_breaker_cooldown": float(os.environ.get("LITIGOS_CB_COOLDOWN", "30.0")),

    # ReAct loop
    "react_max_iterations": int(os.environ.get("LITIGOS_REACT_MAX_ITERATIONS", "10")),
    "react_checkpoint_interval": int(os.environ.get("LITIGOS_REACT_CHECKPOINT_INTERVAL", "300")),

    # Memory
    "memory_prune_days": int(os.environ.get("LITIGOS_MEMORY_PRUNE_DAYS", "30")),
    "memory_prune_probability": float(os.environ.get("LITIGOS_MEMORY_PRUNE_PROB", "0.01")),

    # Orchestrator
    "tier_max_workers": int(os.environ.get("LITIGOS_TIER_MAX_WORKERS", "4")),
    "orchestrator_retry_rounds": int(os.environ.get("LITIGOS_ORCH_RETRY_ROUNDS", "2")),
    "tier_success_threshold": float(os.environ.get("LITIGOS_TIER_SUCCESS_THRESHOLD", "0.5")),
}
