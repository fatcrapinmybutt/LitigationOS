...
class EFilingPrepEngine:
    def __init__(self):
        """Initialize e-filing preparation engine."""
        self.output_dir = LITOS_ROOT / "00_SYSTEM" / "efiling_packets"
        try:
            self.output_dir.mkdir(parents=True, exist_ok=True)
        except OSError as e:
            logger.error("Failed to create efiling output directory: %s", e)
    
    def prepare_packet(self, filing_name, config):
        """Prepare an e-filing packet.
        
        Args:
            filing_name: Name of the filing.
            config: Filing configuration dict with system, stack_path, documents, fee.
            
        Returns:
            dict: Filing result with documents, issues, and ready status.
        """
        if not filing_name or not isinstance(filing_name, str):
            logger.error("Invalid filing_name: %s", filing_name)
            return {"filing": str(filing_name), "ready": False, "issues": ["Invalid filing name"], "documents": []}
        if not config or not isinstance(config, dict):
            logger.error("Invalid config for %s", filing_name)
            return {"filing": filing_name, "ready": False, "issues": ["Invalid config"], "documents": []}
        system_key = config.get("system")
        if system_key not in EFILING_SYSTEMS:
            logger.error("Unknown e-filing system: %s", system_key)
            return {"filing": filing_name, "ready": False, "issues": [f"Unknown system: {system_key}"], "documents": []}
        system = EFILING_SYSTEMS[system_key]
        ...
        try:
            ...
            logger.error("Failed to write checklist for %s: %s", filing_name, e)
    ...
    def prepare_all(self):
        ...
        Returns:
            list: Results for each filing packet.
        """
        results = []
        for name, config in FILING_CONFIGS.items():
            ...
            try:
                ...
                logger.error("Failed to prepare packet %s: %s", name, e)
                result = {"filing": name, "ready": False, "issues": [str(e)], "documents": []}
    ...
    def run(self):
        """Execute full e-filing preparation.
        
        Returns:
            list: Filing packet results, or empty list on failure.
        """
        try:
            return self.prepare_all()
        except Exception as e:
            logger.error("E-filing prep engine failed: %s", e)
            return []
...
if __name__ == "__main__":
    try:
        ...
        logger.error("E-filing prep engine crashed: %s", e)
        print(f"[ERROR] E-filing prep engine crashed: {e}")
