...
class PreFilingQAEngine:
    def __init__(self, stack_name, stack_config):
        """Initialize pre-filing QA engine for a specific filing stack.
        
        Args:
            stack_name: Name of the filing stack.
            stack_config: Configuration dict with path, deadline, court, required_docs.
        """
        if not stack_name or not isinstance(stack_name, str):
            raise ValueError("stack_name must be a non-empty string")
        if not stack_config or not isinstance(stack_config, dict):
            raise ValueError("stack_config must be a non-empty dict")
        if "path" not in stack_config:
            raise ValueError("stack_config must contain 'path' key")
    ...
    def check_placeholders(self):
        ...
        for f in self.path.rglob("*.md"):
            try:
                ...
                logger.debug("Error scanning file for placeholders: %s", e)
                continue
    ...
    def check_signatures(self):
        ...
        for f in self.path.rglob("*.md"):
            try:
                ...
                logger.debug("Error checking signature in file: %s", e)
                continue
    ...
    def check_deadline(self):
        ...
        Returns:
            int: Days remaining until deadline, or 999 on error.
        """
        try:
            today = datetime.now().date()
            deadline_str = self.config.get("deadline", "2099-12-31")
            ...
            logger.error("Failed to parse deadline for %s: %s", self.name, e)
            self.checks["deadline_days"] = 999
            return 999
...
def run_all_qa():
    ...
    for name, config in sorted(FILING_STACKS.items(), key=lambda x: x[1]["deadline"]):
        ...
        try:
            ...
            logger.error("QA failed for %s: %s", name, e)
            verdict = "ERROR"
        print()
        all_results.append({
            "name": name,
...
if __name__ == "__main__":
    try:
        ...
        logger.error("Pre-filing QA sweep failed: %s", e)
        print(f"[ERROR] Pre-filing QA sweep failed: {e}")
