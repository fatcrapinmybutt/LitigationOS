"""
MANBEARPIG v3.0 — 10-Layer Multi-System Litigation Intelligence Matrix
=======================================================================
Queries litigation_context.db for ALL 10 layers:
...
# ═══════════════════════════════════════════════════════════════════════
# CROSS-LAYER LINKS — Wire the 10 layers together
# ═══════════════════════════════════════════════════════════════════════
    ...
    if "docforge" in nmap:
        for n in all_nodes:
            if n["layer"] == "filing":
                ...
                break  # just one representative link
...
<head>
<meta charset="UTF-8">
<title>MANBEARPIG v3.0 — 10-Layer Litigation Intelligence Matrix</title>
...
<div id="viz">
  ...
  <div id="hud">
    <h1>▲ MANBEARPIG v3.0</h1>
    <div class="subtitle">10-LAYER LITIGATION INTELLIGENCE MATRIX — PIGORS v. WATSON</div>
    <div class="stat"><b>__NODECOUNT__</b> nodes · <b>__LINKCOUNT__</b> links · <b>10</b> layers</div>
    <div class="stat">Separation: <b>__SEP__ days</b> · Evidence: <b>__EVCOUNT__</b> quotes</div>
    <div class="stat">Authorities: <b>__AUTHCOUNT__</b> chains · Violations: <b>__JVCOUNT__</b></div>
  </div>
  <div id="controls">
    <h3>▼ LAYERS</h3>
  </div>
...
const LAYER_META = {
  adversary:     {label:"Adversary Network",    icon:"⚔",  color:"#ff0040", y:0.12},
  authority:     {label:"Legal Authority",       icon:"⚖",  color:"#00ffcc", y:0.25},
  filing:        {label:"Filing Pipeline",       icon:"📁", color:"#ffaa00", y:0.38},
  lanes:         {label:"Case Lanes",            icon:"🛤",  color:"#4488ff", y:0.50},
  evidence:      {label:"Evidence Arsenal",      icon:"🔬", color:"#ff2266", y:0.62},
  system:        {label:"System Architecture",   icon:"⚙",  color:"#00ff99", y:0.72},
  fred:          {label:"FRED Governance",       icon:"🏛",  color:"#cc00dd", y:0.15},
  meek:          {label:"MEEK Lane Routing",     icon:"🗺",  color:"#44aaff", y:0.40},
  event_horizon: {label:"Event Horizon Δ∞",     icon:"🌀", color:"#ff6600", y:0.65},
  brains:        {label:"Brain Network",         icon:"🧠", color:"#00ff88", y:0.85},
};
...
def main():
    print("=" * 60)
    print("MANBEARPIG v3.0 — 10-Layer Builder")
    print("=" * 60)
    conn = get_conn()
    ...
    print(f"       {sz:,} bytes | {stats['nodes']} nodes | {stats['links']} links | 10 layers")
