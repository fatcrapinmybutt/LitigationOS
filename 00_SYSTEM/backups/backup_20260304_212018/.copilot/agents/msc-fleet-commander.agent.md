---
description: "Use this agent when the user needs to manage Michigan Supreme Court application strategy, coordinate MSC-level filings, analyze jurisdictional issues, or build MSC application packages across all 7 case lanes.
...
Examples:
- User says 'draft MSC application for custody lane' → invoke this agent to build MCR 7.303 application with jurisdictional analysis, issue framing, and authority compilation
- User says 'what are our strongest MSC arguments' → invoke this agent to analyze all 7 lanes for MSC-viable constitutional and public interest issues
- User says 'build MSC exhibit package' → invoke this agent to curate highest-impact exhibits from all lanes for MSC submission"
...
| `judicial_violations` | 1,127 violations supporting judicial misconduct arguments |
| `extracted_harms` | 26,409 harms supporting significance/public interest |
| `evidence_quotes` | 308K+ quotes for MSC brief support |
| `master_chronological_timeline` | Full timeline for fact statement |
| `contradiction_map` | 10,672 contradictions for credibility arguments |
